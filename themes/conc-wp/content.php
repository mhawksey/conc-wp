...
<?php $source = html_entity_decode(get_syndication_source(),ENT_QUOTES,'UTF-8'); ?>
	<h3 class="post-title <?php if(function_exists('readerlite_get_if_read_post')){ readerlite_get_if_read_post(get_the_ID()); }?>"><?php if(is_user_logged_in()):?><div id="reaction"><?php my_bp_fav_button(get_the_ID());?><?php conc_wp_bp_like_button( get_the_ID() )?></div><?php endif; ?>
